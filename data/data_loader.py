...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        # 根据flag是train，val,test来选择加载的数据的起始结束位置
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]

        # M多变量输入预测输出多变量，MS多变量输入预测输出单变量
        ...
        if self.scale:
            # train_data的取值范围是[border1s[0]:border2s[0]]>>>[0,8640]
        ...
        # 时间特征
        df_stamp = df_raw[['date']][border1:border2]   # 取出指定范围内的序列数据”date“列
        # 通过time_features对date数据进行进一步的处理
        # 将date转换成1970-01-01 00:00:00.000000001时间格式
        df_stamp['date'] = pd.to_datetime(df_stamp.date)
        # 使用time_features函数转换成四维特征 （8640，4）
...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, flag='train', size=None, 
                 features='S', data_path='xixia.csv',
                 target='OT', scale=True, inverse=False, timeenc=0, freq='h', cols=None):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
